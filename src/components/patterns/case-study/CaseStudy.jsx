...
──────────────────────────────────────────────────────────────────────── */
function ScreenClipWipe({ interval = 5200, wipeDuration = 1.6 }) {
  const [active, setActive] = useState(0);
        ...
        {SCREENS.map((screen, i) => {
          ...
          if (isActive) return (
            <div
              ...
              style={{ zIndex: 3, animationDuration: `${wipeDuration}s` }}
            >
              <img src={screen.src} alt={screen.label} className="cs-browser-img" draggable={false} />
            </div>
          );
...
export default function CaseStudy({ screenVariant = 'fade', wipeInterval = 5200, wipeDuration = 1.6 }) {
    ...
    <section className="cs-section">
      ...
      <div ref={wrapperRef} className="cs-media">
        <div ref={innerRef} className="cs-media-inner">
          ...
          {screenVariant === 'wipe'  && <ScreenClipWipe interval={wipeInterval} wipeDuration={wipeDuration} />}
        </div>
      </div>
